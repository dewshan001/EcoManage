...
const RegisterManager = () => {
    ...
    const [editFormData, setEditFormData] = useState({ fullName: '', email: '', contactNumber: '', address: '' });
    ...
    const handleEditSubmit = async (e) => {
        ...
        if (!editFormData.fullName.trim() || !editFormData.email.trim()) {
            showAlert('Full name and email are required.');
            return;
        }

        try {
            const res = await fetch(`http://localhost:5000/api/auth/managers/${managerToEdit.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (res.ok) {
                ...
                showAlert(data.message || 'Failed to update manager.');
            }
        } catch {
            showAlert('Server error while updating manager.');
        }
    };
    ...
    return (
        <div className="rm-root">
            ...
            {/* Edit Manager Modal */}
            {managerToEdit && (
                <div className="rm-modal-backdrop" onClick={() => setManagerToEdit(null)}>
                    <div className="rm-modal-box rm-modal-box-edit" onClick={e => e.stopPropagation()}>
                        <h3 className="rm-edit-title">Edit Garbage Manager</h3>
                        <p className="rm-edit-sub">Update manager details and save your changes.</p>

                        <form className="rm-edit-form" onSubmit={handleEditSubmit}>
                            <div className="rm-form-group">
                                <label>Full Name <span className="req">*</span></label>
                                <input
                                    type="text"
                            ...
                            <div className="rm-modal-actions">
                                <button type="button" className="rm-btn-ghost" onClick={() => setManagerToEdit(null)}>
                                    Cancel
                                </button>
                                <button type="submit" className="rm-btn-solid">
                                    Save Changes
