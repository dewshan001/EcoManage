...
const Workers = () => {
    ...
    const handleEditWorker = (worker) => {
        ...
        setEditWorkerForm({
            ...
            skill: worker.skill || 'General Waste',
            status: worker.status || 'Available'
        });
    };

    const handleEditWorkerFormChange = (e) => {
        const { name, value } = e.target;
        setEditWorkerForm(prev => ({ ...prev, [name]: value }));
    };

    const submitWorkerEdit = async (e) => {
        e.preventDefault();
        if (!workerToEdit) return;

        const normalizedName = editWorkerForm.name.trim();
        const normalizedEmail = editWorkerForm.email.trim().toLowerCase();

        if (!normalizedName || !normalizedEmail) {
            setEditWorkerError('Full name and email are required.');
            return;
        }
        ...
        try {
            const response = await fetch(`http://localhost:5000/api/auth/workers/${workerToEdit.dbId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    fullName: normalizedName,
                    email: normalizedEmail,
                    role: editWorkerForm.role,
                    skill: editWorkerForm.skill,
                    status: editWorkerForm.status
                })
            });
            ...
            const updatedAvatar = updatedWorker.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'W';

            setWorkers(prevWorkers => prevWorkers.map(w =>
                w.dbId === workerToEdit.dbId
                    ? {
                        ...w,
            ...
            setEditWorkerError(error.message || 'Failed to update worker.');
        } finally {
    ...
    return (
        <div className="wm-root">
                                            ...
                                            <tr key={w.id}>
                                                ...
                                                <td>
                                                    <div className="wm-table-actions">
                                                        <button className="wm-btn-edit" onClick={() => handleEditWorker(w)} title="Edit Staff">
                                                            <EditIcon />
                                                        </button>
            ...
            {workerToEdit && (
                <div className="modal-backdrop" onClick={() => setWorkerToEdit(null)}>
                    <div className="modal-box" onClick={e => e.stopPropagation()}>
                        <header className="modal-hdr">
                            <span className="modal-icon-wrap">✏️</span>
                            <div><h3>Edit Worker</h3><p>Update worker details and role settings</p></div>
                            <button className="modal-x" onClick={() => setWorkerToEdit(null)} disabled={editWorkerLoading}>✕</button>
                        </header>
                        <form className="modal-form" onSubmit={submitWorkerEdit}>
                            {editWorkerError && <div style={{ color: 'red', margin: '10px 0', padding: '10px', backgroundColor: '#fee2e2', borderRadius: '4px' }}>{editWorkerError}</div>}
                            <div className="mf-field"><label>Full Name</label>
                                <input name="name" value={editWorkerForm.name} onChange={handleEditWorkerFormChange} required />
                            </div>
                            <div className="mf-field"><label>Email Address</label>
                                <input type="email" name="email" value={editWorkerForm.email} onChange={handleEditWorkerFormChange} required />
                            </div>
                            <div className="mf-row">
                                <div className="mf-field"><label>Role</label>
                                    <select name="role" value={editWorkerForm.role} onChange={handleEditWorkerFormChange}>
                                        {['Collector', 'Driver', 'Supervisor', 'Maintenance'].map(t => <option key={t}>{t}</option>)}
                                    </select>
                                </div>
                                <div className="mf-field"><label>Specialty / Skill</label>
                                    <select name="skill" value={editWorkerForm.skill} onChange={handleEditWorkerFormChange}>
                                        {['General Waste', 'Recycling', 'Hazardous', 'Heavy Vehicle', 'Light Vehicle', 'Field Ops'].map(t => <option key={t}>{t}</option>)}
                                    </select>
                                </div>
                            </div>
                            <div className="mf-field"><label>Status</label>
                                <select name="status" value={editWorkerForm.status} onChange={handleEditWorkerFormChange}>
                                    {['Available', 'On Duty', 'Leave'].map(s => <option key={s}>{s}</option>)}
                                </select>
                            </div>
                            <footer className="modal-footer">
                                <button type="button" className="btn-ghost" onClick={() => setWorkerToEdit(null)} disabled={editWorkerLoading}>Cancel</button>
                                <button type="submit" className="btn-solid" disabled={editWorkerLoading}>{editWorkerLoading ? 'Saving...' : 'Save Changes'}</button>
                            </footer>
                        </form>
                    </div>
                </div>
